...
def test_stencil_accuracy():
    """
    Test the accuracy of different stencils on known functions
    """
    import matplotlib.pyplot as plt
    
    # Create a test function: f(x,y) = sin(πx) * cos(πy)
    x = torch.linspace(0, 2, 100)
    y = torch.linspace(0, 2, 100)
    X, Y = torch.meshgrid(x, y, indexing='ij')
    
    # Test function and its analytical derivatives
    f = torch.sin(torch.pi * X) * torch.cos(torch.pi * Y)
    dfdx_analytical = torch.pi * torch.cos(torch.pi * X) * torch.cos(torch.pi * Y)
    d2fdx2_analytical = -torch.pi**2 * torch.sin(torch.pi * X) * torch.cos(torch.pi * Y)
    
    f = f.unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions
    ...
    for order in orders:
        try:
            # First derivative
            D1 = ConvOperator(domain='x', order=1, taylor_order=order)
            dfdx_numerical = D1(f)[0, 0]
            error_1st = torch.mean((dfdx_numerical - dfdx_analytical[1:-1, 1:-1])**2).item()
            errors_1st.append(error_1st)
            
            # Second derivative  
            D2 = ConvOperator(domain='x', order=2, taylor_order=order)
            d2fdx2_numerical = D2(f)[0, 0]
            error_2nd = torch.mean((d2fdx2_numerical - d2fdx2_analytical[1:-1, 1:-1])**2).item()
            errors_2nd.append(error_2nd)
            
        except ValueError:
            errors_1st.append(float('nan'))
            errors_2nd.append(float('nan'))
    
    print("Accuracy test results:")
    print("Order\t1st Deriv Error\t2nd Deriv Error")
    for i, order in enumerate(orders):
        print(f"{order}\t{errors_1st[i]:.2e}\t\t{errors_2nd[i]:.2e}")

test_stencil_accuracy()
# %%
