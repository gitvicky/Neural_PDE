...
loc = '/pitagora_work/FUPB1_UKAEA_ML/vgopakum/Data/PMocz_NS_Spectral'
# %%
start_time = time()
...
if solver == 'Spectral':
    ...
    while ii < n_sims:
        ...
        if err == 0:
            np.savez(loc + "/NS_Spectral_" + str(ii) + ".npz", u=u[::t_slice, ::x_slice, ::x_slice], v=v[::t_slice, ::x_slice, ::x_slice], p=p[::t_slice, ::x_slice, ::x_slice], w=w[::t_slice, ::x_slice, ::x_slice], x=x[::x_slice], t=0.001*t_slice)
            ii+=1
    ...
    np.savez(loc + "/NS_Spectral_combined_pitagora_OOD_nu_1e-2.npz", u=u, v=v, p=p, w=w, x=x, dt=0.001*t_slice)
...
if solver == 'FV':
    ...
    while ii < n_sims:
        ...
        np.savez(loc + "/NS_FV_" + str(ii) + ".npz", u=uu[::t_slice, ::x_slice, ::x_slice], v=vv[::t_slice, ::x_slice, ::x_slice], p=pp[::t_slice, ::x_slice, ::x_slice], rho=rho[::t_slice, ::x_slice, ::x_slice], dx=dx*x_slice, dtt=dtt[0]*t_slice)
        ii+=1
    ...
    np.savez(loc + "/NS_FV_combined_pitagora_gamma_2by3.npz", u=u, v=v, p=p, rho=rho, dx=dx, dt=dt)
