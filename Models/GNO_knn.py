...
class GNOLayer(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_weights):
        ...
        NOTE: We now expect edge_weights to be pre-calculated and passed in
    ...
    def message(self, x_j, edge_weights):
        ...
        Optimization: Using torch.einsum is often clearer and as-fast-as bmm.
        'bi,bio->bo' translates to:
        Batch (E), In (C_in), Out (C_out)
        Multiply [E, C_in] with [E, C_in, C_out] and sum over C_in
        Result is [E, C_out]
        """
        return torch.einsum('bi,bio->bo', x_j, edge_weights)
        
        # Original (also correct, but einsum is clean):
...
class GNO(nn.Module):
    """
    Optimized GNO model with k-NN graph and true batching
    """
    def __init__(self, in_channels=1, out_channels=1, hidden_channels=32, n_layers=4, k_neighbours=20,
                 x_in=None, y_in=None, x_out=None, y_out=None):
        super().__init__()
        
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.k_neighbours = k_neighbours # Changed from 'r'
        ...
        # Build graph - this is the unbatched version, used as a template
        # We register them as buffers so they are moved to .cuda() / .cpu()
        ...
        if not self.same_grid:
            raise NotImplementedError("Batched different-grid GNO is more complex. "
                                      "This example focuses on same-grid optimization.")
    
    def _build_graph(self):
        """
        Optimization: Build a k-NN graph instead of a radius graph.
        """
        if self.same_grid:
            # Use k-NN graph. E = N * k
            edge_index = knn_graph(self.coords_in, k=self.k_neighbours, loop=False)
            # Create edge attributes
            edge_attr = torch.cat([self.coords_in[edge_index[0]], self.coords_in[edge_index[1]]], dim=-1)
        else:
            # This part remains complex; sticking to same_grid optimization for now
            pwd = torch.cdist(self.coords_in, self.coords_out)
            edge_index = torch.stack(torch.where(pwd <= (self.k_neighbours / self.N_in) )) # Heuristic r
    ...
    def forward(self, u):
        ...
        if not self.same_grid:
            # The batched version for different grids is complex and needs
            # careful handling of combined node sets.
            # The original loop-based code would still work here,
            # but would be slow.
            print("Warning: Using slow, unbatched forward pass for different grids.")
            outputs = []
            ...
            for b in range(batch_size):
                ...
                # Note: self.base_edge_index/attr are for the different-grid case
                x_out = self.gno_block(x_combined, self.base_edge_index, self.base_edge_attr) 
            ...
            # 2. Create batched edge_index and edge_attr
            # This is the most complex part of PyG batching.
            # We create B copies of the graph and offset the node indices.
            
            # Create B offsets: [0, N, 2*N, 3*N, ...]
            offsets = torch.arange(batch_size, device=x.device) * self.N_in
            # Add offsets to edge_index
            # [2, E] + [B, 1] -> broadcast to [B, 2, E] -> reshape to [2, B*E]
            batched_edge_index = self.base_edge_index.unsqueeze(0) + offsets.view(-1, 1, 1)
            batched_edge_index = batched_edge_index.reshape(2, -1)
            
            # Repeat edge_attr B times
            batched_edge_attr = self.base_edge_attr.repeat(batch_size, 1)
            
            # 3. Call GNO block ONCE
        ...
        # Reshape to output image format
...
#         hidden_channels=16,
#         n_layers=2,
#         k_neighbours=20, # Use k-NN instead of radius
#         x_in=x,
#         y_in=y
#     )
...
#     print(f"Graph: k-NN with k={model.k_neighbours}")
#     print(f"Nodes (N): {model.N_in:,}")
#     print(f"Edges (E) per sample: {model.base_edge_index.shape[1]:,}")
